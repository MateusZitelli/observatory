      ...
      const bindings = [
        // Main sliders
        { id: "paramH", key: "H_con", type: "range", valId: "valH", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramPierD", key: "pierD", type: "range", valId: "valPierD", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramExtH", key: "H_ext", type: "range", valId: "valExtH", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramPivotX", key: "X_PIVOT", type: "range", valId: "valPivotX", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramMountY", key: "Y_MOUNT", type: "range", valId: "valMountY", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramLat", key: "lat", type: "range", valId: "valLat", fmt: v => parseFloat(v).toFixed(1) + "°" },
        { id: "paramBaseY", key: "Y_BASE", type: "range", valId: "valBaseY", fmt: v => parseFloat(v).toFixed(3) + " m" },
        { id: "paramRaZ", key: "Z_RA", type: "range", valId: "valRaZ", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramDecY", key: "Y_DEC", type: "range", valId: "valDecY", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramCwY", key: "Y_CW", type: "range", valId: "valCwY", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramTubeLen", key: "TUBE_LEN", type: "range", valId: "valTubeLen", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramTubeOff", key: "TUBE_OFF", type: "range", valId: "valTubeOff", fmt: v => parseFloat(v).toFixed(3) + " m" },
        { id: "paramEyeLen", key: "EYE_LEN", type: "range", valId: "valEyeLen", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramTubeD", key: "TUBE_D", type: "range", valId: "valTubeD", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomW", key: "rW", type: "range", valId: "valRoomW", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomD", key: "rD", type: "range", valId: "valRoomD", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomH", key: "rH", type: "range", valId: "valRoomH", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoofOpen", key: "roofOpen", type: "range", valId: "valRoofOpen", fmt: v => Math.round(parseFloat(v)) + "%" },
        { id: "paramRoofPitch", key: "roofPitch", type: "range", valId: "valRoofPitch", fmt: v => Math.round(parseFloat(v)) + "°" },
        { id: "paramRA", key: "RA", type: "range", valId: "valRA", fmt: v => Math.round(parseFloat(v)) + "°" },
        { id: "paramDec", key: "Dec", type: "range", valId: "valDec", fmt: v => Math.round(parseFloat(v)) + "°" },
        ...
        // Selects
        { id: "paramRoofDir", key: "roofDir", type: "select", valId: "valRoofDir", fmt: v => v },
        { id: "observerPosture", key: "observerPosture", type: "select" },
        // Sky mirrors (same state key, different DOM element)
        { id: "paramHSky", key: "H_con", type: "range", valId: "valHSky", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomHSky", key: "rH", type: "range", valId: "valRoomHSky", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomWSky", key: "rW", type: "range", valId: "valRoomWSky", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoomDSky", key: "rD", type: "range", valId: "valRoomDSky", fmt: v => parseFloat(v).toFixed(2) + " m" },
        { id: "paramRoofPitchSky", key: "roofPitch", type: "range", valId: "valRoofPitchSky", fmt: v => Math.round(parseFloat(v)) + "°" },
        { id: "paramRoofOpenSky", key: "roofOpen", type: "range", valId: "valRoofOpenSky", fmt: v => Math.round(parseFloat(v)) + "%" },
        // Sky-only params
        { id: "paramSkyHour", key: "skyHour", type: "range", valId: "valSkyHour", fmt: function(v) { v = parseFloat(v); if (v < 0) return "agora"; const h = Math.floor(v), m = Math.round((v-h)*60); return String(h).padStart(2,"0")+":"+String(m).padStart(2,"0"); } },
        { id: "paramSkyDay", key: "skyDay", type: "range", valId: "valSkyDay", fmt: function(v) { v = parseInt(v); if (v < 0) return "hoje"; const d = new Date(new Date().getFullYear(), 0, v); return String(d.getDate()).padStart(2,"0")+"/"+String(d.getMonth()+1).padStart(2,"0"); } },
        { id: "paramPanoRot", key: "panoRot", type: "range", valId: "valPanoRot", fmt: v => Math.round(parseFloat(v)) + "°" },
      ];

      const LS_KEY_V2 = "observatorio_state_v2";

      function syncFromDOM(binding) {
        const el = document.getElementById(binding.id);
        if (!el) return;
        if (binding.type === "checkbox") state[binding.key] = el.checked;
        else if (binding.type === "select") state[binding.key] = el.value;
        else state[binding.key] = parseFloat(el.value);
      }
      ...
      function syncAllToDOM() {
        for (const b of bindings) {
          ...
          if (!el) continue;
          if (b.type === "checkbox") el.checked = state[b.key];
          else el.value = state[b.key];
          if (b.valId && b.fmt) {
            const valEl = document.getElementById(b.valId);
            if (valEl) valEl.innerText = b.fmt(state[b.key]);
          }
        }
      }
      ...
      function computeDerived() {
        const s = state;
        derived.H_total = s.H_con + s.H_ext + s.Y_MOUNT;
        derived.TUBE_R = s.TUBE_D / 2;
        derived.Z_FRONT = s.TUBE_LEN / 2 + s.TUBE_OFF;
        derived.Z_BACK = s.TUBE_LEN / 2 - s.TUBE_OFF;
        derived.Z_BACK_TOTAL = derived.Z_BACK + s.EYE_LEN;
        derived.PITCH_TAN = Math.tan(s.roofPitch * Math.PI / 180);
        const ridgeX = (s.roofDir === "N" || s.roofDir === "S");
        derived.halfSpan = (ridgeX ? s.rD : s.rW) / 2 + 0.15;
        derived.ridgeRise = derived.halfSpan * derived.PITCH_TAN;
        derived.ridgeH = s.rH + derived.ridgeRise;
      }
      ...
      const btnTabRoom = document.getElementById("btnTabRoom");
      const btnTab2D = document.getElementById("btnTab2D");
      const btnTabSky = document.getElementById("btnTabSky");
      const btnTabProj = document.getElementById("btnTabProj");

      const content3D = document.getElementById("content-3D");
      const contentROOM = document.getElementById("content-ROOM");
      const content2D = document.getElementById("content-2D");
      const contentSKY = document.getElementById("content-SKY");
      const contentPROJ = document.getElementById("content-PROJ");
      const motorsBlock = document.getElementById("motors-block");
      const canvasContainer = document.getElementById("canvas-container");
      const canvas2D = document.getElementById("canvas-2d");

      function switchTab(tabStr) {
        currentTab = tabStr;
        is2DMode = tabStr === "2D" || tabStr === "SKY" || tabStr === "PROJ";

        btnTab3D.classList.replace("bg-blue-600", "bg-gray-700");
        btnTabRoom.classList.replace("bg-blue-600", "bg-gray-700");
        btnTab2D.classList.replace("bg-blue-600", "bg-gray-700");
        btnTabSky.classList.replace("bg-blue-600", "bg-gray-700");
        btnTabProj.classList.replace("bg-blue-600", "bg-gray-700");

        content2D.classList.add("hidden-content");
        content3D.classList.add("hidden-content");
        contentROOM.classList.add("hidden-content");
        contentSKY.classList.add("hidden-content");
        contentPROJ.classList.add("hidden-content");
        motorsBlock.classList.add("hidden-content");

        if (tabStr === "2D") {
          btnTab2D.classList.replace("bg-gray-700", "bg-blue-600");
          content2D.classList.remove("hidden-content");
          canvasContainer.style.display = "none";
          canvas2D.style.display = "block";
          draw2D();
        } else if (tabStr === "SKY") {
          btnTabSky.classList.replace("bg-gray-700", "bg-blue-600");
          contentSKY.classList.remove("hidden-content");
          canvasContainer.style.display = "none";
          canvas2D.style.display = "block";
          drawSky();
        } else if (tabStr === "PROJ") {
          btnTabProj.classList.replace("bg-gray-700", "bg-blue-600");
          contentPROJ.classList.remove("hidden-content");
          canvasContainer.style.display = "none";
          canvas2D.style.display = "block";
          drawProject();
        } else if (tabStr === "ROOM") {
          btnTabRoom.classList.replace("bg-gray-700", "bg-blue-600");
          contentROOM.classList.remove("hidden-content");
          motorsBlock.classList.remove("hidden-content");
          canvas2D.style.display = "none";
          canvasContainer.style.display = "block";
          if (roomGroup) roomGroup.visible = true;
        } else {
          btnTab3D.classList.replace("bg-gray-700", "bg-blue-600");
          content3D.classList.remove("hidden-content");
          motorsBlock.classList.remove("hidden-content");
          canvas2D.style.display = "none";
          canvasContainer.style.display = "block";
          if (roomGroup) roomGroup.visible = false;
        }
        updateAll(); // Força a atualização da visibilidade dos móveis
      }

      btnTab3D.addEventListener("click", () => switchTab("3D"));
      btnTabRoom.addEventListener("click", () => switchTab("ROOM"));
      btnTab2D.addEventListener("click", () => switchTab("2D"));
      btnTabSky.addEventListener("click", () => switchTab("SKY"));
      btnTabProj.addEventListener("click", () => switchTab("PROJ"));
      ...
      let volumeMesh,
        ...
        observerGroup,
        roomGroup;
      let furnitureGroup, deskGroup, chairGroup, sofaGroup, sofaBedMat, mattressGroup;
      let archGroup, doorMesh, windowMesh, wallFrameGroup;
      let roofGroup;

      let rigGroup, tiltGroup, raNode, decHousingGroup, decNode;
      let baseBlock,
        baseDial,
        baseRod,
        raHousing,
      ...
      const btnIdealPier = document.getElementById("btnIdealPier");
      ...
      function init() {
        ...
        btnIdealPier.addEventListener("click", () => {
          const targetEyeMinZ = state.observerPosture === "sitting" ? 0.65 : 1.25;
          const diff = targetEyeMinZ - derived.currentEyeMinZ;
          let newH = state.H_con + diff;
      ...
      function createFurniture() {
        ...
        const fabricChairMat = new THREE.MeshStandardMaterial({
          color: 0x1f2937,
          roughness: 0.8,
        });
        ...
        const legGeo = new THREE.BoxGeometry(0.05, 0.725, 0.05);
        const legsPos = [
          [-0.75, -0.3],
          [0.75, -0.3],
          [-0.75, 0.3],
        ...
        // 3. Sofá-cama (1.80m x 0.95m fechado, assento 0.42m, total ~0.90m)
        sofaGroup = new THREE.Group();
        const sSeat = new THREE.Mesh(
          new THREE.BoxGeometry(1.80, 0.42, 0.95),
          fabricSofaMat,
        );
        sSeat.position.set(0, 0.21, 0);
        sofaGroup.add(sSeat);
        const sBack = new THREE.Mesh(
          new THREE.BoxGeometry(1.80, 0.48, 0.18),
          fabricSofaMat,
        );
        sBack.position.set(0, 0.66, -0.385);
        sofaGroup.add(sBack);
        const armL = new THREE.Mesh(
          new THREE.BoxGeometry(0.18, 0.55, 0.95),
          fabricSofaMat,
        );
        ...
        sofaGroup.add(armR);
        // Colchão do sofá-cama (extensão frontal, visível quando aberto)
        sofaBedMat = new THREE.Group();
        const bedSurface = new THREE.Mesh(
          new THREE.BoxGeometry(1.80, 0.15, 1.05),
        ...
        const p1 = new THREE.Mesh(
          new THREE.BoxGeometry(0.55, 0.08, 0.35), pillowMat,
        );
        ...
        // Colchão queen no chão (1.58m x 1.98m x 0.20m)
        mattressGroup = new THREE.Group();
        const mattBase = new THREE.Mesh(
          new THREE.BoxGeometry(1.58, 0.15, 1.98),
          new THREE.MeshStandardMaterial({ color: 0xf5f5f0, roughness: 0.9 }),
        );
        mattBase.position.set(0, 0.075, 0);
        mattressGroup.add(mattBase);
        const mPillow1 = new THREE.Mesh(
          new THREE.BoxGeometry(0.55, 0.08, 0.35), pillowMat,
        );
        mPillow1.position.set(-0.35, 0.19, -0.75);
        mattressGroup.add(mPillow1);
        const mPillow2 = new THREE.Mesh(
          new THREE.BoxGeometry(0.55, 0.08, 0.35), pillowMat,
        );
        mPillow2.position.set(0.35, 0.19, -0.75);
        mattressGroup.add(mPillow2);
        furnitureGroup.add(mattressGroup);

        scene.add(furnitureGroup);
        furnitureGroup.visible = false;

        // 4. Estrutura wood frame (criada vazia, populada por buildWallFrame)
        wallFrameGroup = new THREE.Group();
        scene.add(wallFrameGroup);
        wallFrameGroup.visible = false;

        // 5. Arquitetura (Porta e Janela)
        archGroup = new THREE.Group();

        doorMesh = new THREE.Group();
        const doorFrame = new THREE.Mesh(
          new THREE.BoxGeometry(0.05, 2.1, 0.9),
          woodMat,
        );
        ...
        // 5. Telhado de duas águas (telhas sanduíche 3m x 1m, dinâmico)
      ...
      function createMeshes() {
        // Room
        const roomGeo = new THREE.BoxGeometry(1, 1, 1);
        ...
        const rMesh = new THREE.Mesh(roomGeo, roomMat);
        ...
        const rEdges = new THREE.LineSegments(edgesGeo, edgesMat);

        roomGroup = new THREE.Group();
        roomGroup.add(rMesh);
        roomGroup.add(rEdges);
        scene.add(roomGroup);
        roomGroup.visible = false;

        // Pier & Mount
        pierMesh = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          new THREE.MeshStandardMaterial({ color: 0xd1d5db, roughness: 0.9 }),
        );
        pierMesh.geometry.translate(0, 0.5, 0);
        scene.add(pierMesh);

        pierExtMesh = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
        ...
        pierExtMesh.geometry.translate(0, 0.5, 0);
        scene.add(pierExtMesh);

        mountBaseMesh = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          new THREE.MeshStandardMaterial({
            color: 0x18181b,
            roughness: 0.8,
        ...
        mountBaseMesh.geometry.translate(0, 0.5, 0);
        scene.add(mountBaseMesh);
        ...
        const bodyMesh = new THREE.Mesh(
          new THREE.CylinderGeometry(0.2, 0.2, 1.3, 16),
          obsMat,
        );
        bodyMesh.geometry.translate(0, 0.65, 0);
        const headMesh = new THREE.Mesh(
          new THREE.SphereGeometry(0.15, 16, 16),
          obsMat,
        );
        headMesh.geometry.translate(0, 1.45, 0);
        observerGroup.add(bodyMesh, headMesh);
        scene.add(observerGroup);

        rigGroup = new THREE.Group();
        rigGroup.rotation.x = -Math.PI / 2;
        scene.add(rigGroup);
        ...
        rigGroup.add(tiltGroup);

        baseBlock = new THREE.Mesh(
          new THREE.BoxGeometry(0.18, 0.22, 0.18),
          matBlack,
        );
        tiltGroup.add(baseBlock);
        baseDial = new THREE.Mesh(
          new THREE.CylinderGeometry(0.06, 0.06, 0.19, 32),
          matOrange,
        );
        baseDial.rotation.z = Math.PI / 2;
        tiltGroup.add(baseDial);

        baseRod = new THREE.Mesh(
          new THREE.CylinderGeometry(0.08, 0.09, 1, 32),
          matBlack,
        );
        tiltGroup.add(baseRod);
        raHousing = new THREE.Mesh(
          new THREE.CylinderGeometry(0.1, 0.11, 1, 32),
          matBlack,
        );
        raHousing.rotation.x = Math.PI / 2;
        tiltGroup.add(raHousing);
        controlPanel = new THREE.Mesh(
          new THREE.BoxGeometry(0.14, 0.16, 0.04),
          matDarkGrey,
        );
        controlPanel.rotation.y = Math.PI / 6;
        tiltGroup.add(controlPanel);

        raNode = new THREE.Group();
        tiltGroup.add(raNode);
        decHousingGroup = new THREE.Group();
        raNode.add(decHousingGroup);

        decHousing = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          matBlack,
        );
        decHousing.rotation.z = Math.PI / 2;
        decHousingGroup.add(decHousing);
        decRing = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          matOrange,
        );
        decRing.rotation.z = Math.PI / 2;
        decHousingGroup.add(decRing);

        decNode = new THREE.Group();
        decHousingGroup.add(decNode);
        saddle = new THREE.Group();
        const saddleBlock = new THREE.Mesh(
          new THREE.BoxGeometry(1, 1, 0.12),
          matBlack,
        );
        saddle.add(saddleBlock);
        decNode.add(saddle);
        dovetailBar = new THREE.Mesh(
          new THREE.BoxGeometry(1, 1, 0.06),
          matOrange,
        );
        decNode.add(dovetailBar);

        mainTube = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          matBlack,
        );
        decNode.add(mainTube);
        rearCell = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 0.8, 1, 32),
          matBlack,
        );
        decNode.add(rearCell);
        visualBack = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 16),
          matSilver,
        );
        decNode.add(visualBack);
        dewShield = new THREE.Mesh(
          new THREE.CylinderGeometry(1, 1, 1, 32),
          matDarkGrey,
        );
        decNode.add(dewShield);

        cwShaft = new THREE.Mesh(
          new THREE.CylinderGeometry(0.015, 0.015, 1, 16),
          matSilver,
        );
        cwShaft.rotation.z = Math.PI / 2;
        decNode.add(cwShaft);
        cwWeightsGroup = new THREE.Group();
        ...
        cwWeightsGroup.add(w1);
        const w2 = new THREE.Mesh(
          new THREE.CylinderGeometry(0.08, 0.08, 0.06, 32),
          matBlack,
        );
        w2.rotation.z = Math.PI / 2;
        w2.position.set(-0.065, 0, 0);
        cwWeightsGroup.add(w2);
      }

      function generateKinematicGeometry(
        H,
        Y_DEC,
        Z_length,
        lat,
        ...
        for (let i = 0; i <= raSegments; i++) {
          ...
          for (let j = 0; j <= decSegments; j++) {
            ...
            const x_dec = Y_DEC;
            const y_dec = direction * Z_length;
            const z_dec = 0;

            const x_ra0 = x_dec;
            const y_ra0 = y_dec * Math.cos(dec) - z_dec * Math.sin(dec);
            const z_ra0 = y_dec * Math.sin(dec) + z_dec * Math.cos(dec);
            ...
            const y_tilt = x_ra * Math.sin(ra) + y_ra * Math.cos(ra) + Y_BASE;
            const z_tilt = z_ra;
            ...
            const y_final = z_world0 + H;
            const z_final = -y_world0 - X_PIVOT;

            vertices.push(x_final, y_final, z_final);
          }
        }
        ...
        for (let i = 0; i < raSegments; i++) {
          for (let j = 0; j < decSegments; j++) {
            const a = i * (decSegments + 1) + j,
              b = i * (decSegments + 1) + j + 1;
            const c = (i + 1) * (decSegments + 1) + j,
              d = (i + 1) * (decSegments + 1) + j + 1;
            indices.push(a, b, d);
            indices.push(a, d, c);
          }
        }
        ...
        for (let i = 0; i < posAttr.count; i++) {
          ...
          const fy = py + ny * collisionRadius;
          const fz = pz + nz * collisionRadius;
        ...
        return { geometry: geo, maxZ, minZ, maxRadius };
      }

      function checkPierCrash() {
        ...
        const checkPoint = (localPt, radius) => {
          const worldPt = decNode.localToWorld(localPt.clone());

          if (worldPt.y < 0) return false;

          // 1. Verificar o Pilar de Concreto e Extensão Metálica
          let centerZ = 0;
          let currentRadius = pierR;

          if (worldPt.y > H_con) {
            centerZ = -X_PIVOT;
            currentRadius = extR;

            if (worldPt.y > H_con + H_ext) {
              currentRadius = 0.085;
            }
          }

          const dx = worldPt.x - 0;
          const dz = worldPt.z - centerZ;
          const dist = Math.sqrt(dx * dx + dz * dz);

          if (worldPt.y <= H_con + H_ext + Y_MOUNT + radius) {
            if (dist < currentRadius + radius) return true;
          }

          // 2. Verificar Mobília (Sofá e Mesa)
          if (currentTab === "ROOM" && state.showFurniture) {
            // Sofá-cama (Parede Sul +Z, encostado na Leste +X)
            if (
              worldPt.y < 0.9 + radius &&
              worldPt.x > rW / 2 - 1.9 - radius &&
              worldPt.z > rD / 2 - 1.0 - radius
            ) {
              return true;
            }

            // Mesa + Cadeira (Canto Noroeste -X, -Z)
            if (
              worldPt.y < 1.1 + radius &&
              worldPt.x < -rW / 2 + 1.3 + radius &&
              worldPt.z < -rD / 2 + 1.6 + radius
            ) {
              return true;
            }
          }
        ...
        for (let t = -Z_FRONT; t <= Z_BACK + EYE_LEN; t += 0.05) {
          let r = TUBE_R;
          if (t > Z_BACK) r = 0.03;
          if (checkPoint(new THREE.Vector3(Y_DEC, t, 0), r)) {
            isColliding = true;
            break;
          }
        }
        ...
        if (!isColliding) {
          for (let t = 0; t <= Y_CW; t += 0.05) {
            let r = 0.015;
            if (t > Y_CW - 0.15) r = 0.08;
            if (checkPoint(new THREE.Vector3(-t, 0, 0), r)) {
              isColliding = true;
              break;
            }
          }
