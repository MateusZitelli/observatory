...
function drawTrussBraces(view: SectionView): void {
  const { ctx, toS, halfSpan, rH, ridgeRise } = view;
  for (const sx of [-1, 1]) {
...
function drawTrussColumns(view: SectionView): void {
  const { ctx, toS, halfSpan, rH, roofPeakH, scale } = view;
...
function drawTrussLines(view: SectionView): void {
  const { ctx, toS, halfSpan, rH, roofPeakH, scale } = view;
  drawTrussColumns(view);
  ...
  drawTrussBraces(view);
}

function drawTrussBlocks(view: SectionView): void {
  const { ctx, toS, halfSpan, rH, roofPeakH, ridgeRise } = view;
  ctx.fillStyle = "#c4a35a"; ctx.strokeStyle = "#8b7332"; ctx.lineWidth = 1;
...
function drawTrussLabels(view: SectionView): void {
  const { ctx, toS, halfSpan, rH, roofPeakH, ridgeRise } = view;
  ctx.fillStyle = "#c4a35a"; ctx.font = "8px sans-serif"; ctx.textAlign = "left";
...
export function drawTransverseTruss(view: SectionView): void {
  drawTrussLines(view);
  drawTrussBlocks(view);
  drawTrussLabels(view);
}
